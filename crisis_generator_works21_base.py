...
def replica_catalog(train_tweets_path, val_tweets_path, test_tweets_path, dataset_images, EMBEDDING_BASE_PATH):
    ...
    """
    rc = ReplicaCatalog()
    http_location = "http://panorama-data/~panorama/data/CrisisComputing"
    ...
    for image_path in dataset_images:
        ...
        rc.add_replica("local", image_file, os.path.join(http_location, path_split[-3], path_split[-2], path_split[-1]))
    ...
    rc.add_replica("local", train_tweets_name, os.path.join(http_location, path_split[-2], path_split[-1]))
    path_split = val_tweets_path.split('/')
    rc.add_replica("local", val_tweets_name, os.path.join(http_location, path_split[-2], path_split[-1]))
    path_split = test_tweets_path.split('/')
    rc.add_replica("local", test_tweets_name, os.path.join(http_location, path_split[-2], path_split[-1]))
    
    rc.add_replica("local", glove_embeddings, os.path.join(http_location, "glove_twitter", GLOVE_EMBEDDING_FILE))          
    rc.write()
...
def transformation_catalog():
    ...
    crisis_container = Container(
                ...
                image = "http://panorama-data/~panorama/containers/crisis-computing_latest.sif",
                image_site = "local"
    ...
    preprocess_images = Transformation(
                            "preprocess_images",
                            site = "local",
                            pfn = "http://panorama-data/~panorama/bin/CrisisComputing/preprocess_images.py", 
                            is_stageable = True,
                            container=crisis_container
                        )\
                        .add_pegasus_profiles(label="image_pipeline")

    preprocess_tweets = Transformation(
                            "preprocess_tweets",
                            site = "local",
                            pfn = "http://panorama-data/~panorama/bin/CrisisComputing/preprocess_tweets.py", 
                            is_stageable = True,
                            container=crisis_container
                        )\
                        .add_pegasus_profiles(label="text_pipeline")
    ...
    hpo_train_resnet = Transformation(
                        "hpo_train_resnet",
                        site = "local",
                        pfn = "http://panorama-data/~panorama/bin/CrisisComputing/hpo_train_resnet.py",
                        is_stageable = True,
                        container=crisis_container
                  )\
                  .add_pegasus_profiles(label="image_pipeline", cores=24, gpus=1, memory=131072, runtime=43200, grid_start_arguments="-G -m 10")\
                  .add_env(key="KICKSTART_MON_GRAPHICS_PCIE", value="TRUE")

    train_resnet = Transformation(
                        "train_resnet",
                        site = "local",
                        pfn = "http://panorama-data/~panorama/bin/CrisisComputing/train_resnet.py",
                        is_stageable = True,
                        container=crisis_container
                  )\
                  .add_pegasus_profiles(label="image_pipeline", cores=24, gpus=1, memory=131072, runtime=43200, grid_start_arguments="-G -m 10")\
    ...
    resnet_inference = Transformation(
                        "resnet_inference",
                        site = "local",
                        pfn = "http://panorama-data/~panorama/bin/CrisisComputing/resnet_inference.py",
                        is_stageable = True,
                        container=crisis_container
                  )\
                  .add_pegasus_profiles(label="image_pipeline", cores=24, gpus=1, memory=131072, runtime=43200, grid_start_arguments="-G -m 10")\
    ...
    hpo_train_bilstm = Transformation(
                        "hpo_train_bilstm",
                        site = "local",
                        pfn = "http://panorama-data/~panorama/bin/CrisisComputing/hpo_train_bilstm.py",
                        is_stageable = True,
                        container=crisis_container
                  )\
                  .add_pegasus_profiles(label="text_pipeline", cores=24, gpus=1, memory=131072, runtime=43200, grid_start_arguments="-G -m 10")\
                  .add_env(key="KICKSTART_MON_GRAPHICS_PCIE", value="TRUE")

    train_bilstm = Transformation(
                        "train_bilstm",
                        site = "local",
                        pfn = "http://panorama-data/~panorama/bin/CrisisComputing/train_bilstm.py",
                        is_stageable = True,
                        container=crisis_container
                  )\
                  .add_pegasus_profiles(label="text_pipeline", cores=24, gpus=1, memory=131072, runtime=43200, grid_start_arguments="-G -m 10")\
    ...
    bilstm_inference = Transformation(
                        "bilstm_inference",
                        site = "local",
                        pfn = "http://panorama-data/~panorama/bin/CrisisComputing/bilstm_inference.py",
                        is_stageable = True,
                        container=crisis_container
                  )\
                  .add_pegasus_profiles(label="text_pipeline", cores=24, gpus=1, memory=131072, runtime=43200, grid_start_arguments="-G -m 10")\
    ...
    late_fusion = Transformation(
                        "late_fusion",
                        site = "local",
                        pfn = "http://panorama-data/~panorama/bin/CrisisComputing/late_fusion.py",
                        is_stageable = True,
                        container=crisis_container
                  )\
                  .add_pegasus_profiles(cores=24, gpus=1, memory=131072, runtime=43200, grid_start_arguments="-G -m 10")\
                  .add_env(key="KICKSTART_MON_GRAPHICS_PCIE", value="TRUE")
