...
from supcon_util import TwoCropTransform, AverageMeter
...
CHECKPOINT =  'checkpoint_supcon.pth'
FINAL_OUTPUT = 'supcon_final_model.pth'
...
def parse_option():
    ...
    parser.add_argument('--mean', type=str, default='(0.4905, 0.4729, 0.4560)', help='mean of dataset in path in form of str tuple')
    parser.add_argument('--std', type=str, default='(0.2503, 0.2425, 0.2452)', help='std of dataset in path in form of str tuple')
...
def get_data(PATH):
    ...
    images = glob.glob(PATH+'*.png') + glob.glob(PATH+'*.jpg')
    return images
...
def set_loader(opt):
    ...
    if opt.dataset == 'cifar10':
        ...
        image_data = get_data(opt.data_folder)
        train_dataset = DatasetLoader(image_data, TwoCropTransform(train_transform))
    else:
