...
MODEL_CHECKPOINT = 'bert_model.pth'
...
def get_embeddings(dataloader, model):
    ...
    with torch.no_grad(): 
        ...
        for batch in dataloader:
            ...
            embed.append(sentence_embedding.cpu().numpy())
            id_embed.append(b_input_ids.cpu().numpy())

    tweet_embeddings = [list(item) for item in np.vstack(embed)]
    id_embeddings = [list(item) for item in np.vstack(id_embed)]

    return tweet_embeddings, true_label
