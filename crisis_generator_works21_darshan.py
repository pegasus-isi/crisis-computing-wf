...
def replica_catalog(train_tweets_path, val_tweets_path, test_tweets_path, dataset_images, EMBEDDING_BASE_PATH):
    ...
    rc.add_replica("condorpool", "preprocess_images.py", "/nfs/shared/panorama/bin/CrisisComputing/preprocess_images.py")
    rc.add_replica("condorpool", "preprocess_tweets.py", "/nfs/shared/panorama/bin/CrisisComputing/preprocess_tweets.py")
    rc.add_replica("condorpool", "hpo_train_resnet.py", "/nfs/shared/panorama/bin/CrisisComputing/hpo_train_resnet.py")
    rc.add_replica("condorpool", "train_resnet.py", "/nfs/shared/panorama/bin/CrisisComputing/train_resnet.py")
    rc.add_replica("condorpool", "resnet_inference.py", "/nfs/shared/panorama/bin/CrisisComputing/resnet_inference.py")
    rc.add_replica("condorpool", "hpo_train_bilstm.py", "/nfs/shared/panorama/bin/CrisisComputing/hpo_train_bilstm.py")
    rc.add_replica("condorpool", "train_bilstm.py", "/nfs/shared/panorama/bin/CrisisComputing/train_bilstm.py")
    rc.add_replica("condorpool", "bilstm_inference.py", "/nfs/shared/panorama/bin/CrisisComputing/bilstm_inference.py")
    rc.add_replica("condorpool", "late_fusion.py", "/nfs/shared/panorama/bin/CrisisComputing/late_fusion.py")
...
def transformation_catalog():
    ...
    preprocess_images = Transformation(
                            "preprocess_images",
                            site = "condorpool",
                            pfn = "file:///nfs/shared/panorama/bin/CrisisComputing/darshan_wrapper.sh", 
                            is_stageable = False,
...
def run_workflow(EMBEDDING_BASE_PATH):
    ...
    preprocess_tweets_file = File('preprocess_tweets.py')
    
    job_preprocess_tweets = [Job(preprocess_tweets) for i in range(3)]
    job_preprocess_tweets[0].add_inputs(preprocess_tweets_file, train_tweets)
    job_preprocess_tweets[0].add_outputs(preprocessed_train_tweets)
    job_preprocess_tweets[0].add_args(preprocess_tweets_file, '--filename', 'train_tweets.csv')
        
    job_preprocess_tweets[1].add_inputs(preprocess_tweets_file, val_tweets)
    job_preprocess_tweets[1].add_outputs(preprocessed_val_tweets)
    job_preprocess_tweets[1].add_args(preprocess_tweets_file, '--filename', 'val_tweets.csv')
    
    job_preprocess_tweets[2].add_inputs(preprocess_tweets_file, test_tweets)
    job_preprocess_tweets[2].add_outputs(preprocessed_test_tweets)
    job_preprocess_tweets[2].add_args(preprocess_tweets_file, '--filename', 'test_tweets.csv')


    # Job 2: HPO Bi-LSTM
    bilstm_best_params = File('best_bilstm_hpo_params.txt')
    hpo_train_bilstm_file = File("hpo_train_bilstm.py")
    job_hpo_train_bilstm = Job(hpo_train_bilstm)\
                            .add_inputs(hpo_train_bilstm_file, glove_embeddings, preprocessed_train_tweets, preprocessed_val_tweets, preprocessed_test_tweets)\
                            .add_outputs(bilstm_best_params)\
                            .add_args(hpo_train_bilstm_file, '--trials', BILSTM_NUM_TRIALS)
    ...
    train_bilstm_file = File("train_bilstm.py")
    job_train_bilstm = Job(train_bilstm)\
                        .add_args(train_bilstm_file)\
                        .add_inputs(train_bilstm_file, glove_embeddings, preprocessed_train_tweets, preprocessed_val_tweets, preprocessed_test_tweets, bilstm_best_params)\
    ...
    bilstm_inference_file = File('bilstm_inference.py')
    job_bilstm_inference = Job(bilstm_inference)\
                            .add_args(bilstm_inference_file)\
                            .add_inputs(bilstm_inference_file, preprocessed_train_tweets, preprocessed_val_tweets, preprocessed_test_tweets, trained_bilstm_model)\
    ...
    preprocess_images_file = File("preprocess_images.py")
    for j in job_preprocess_images:
        j.add_args(preprocess_images_file)
        j.add_inputs(preprocess_images_file)

    # Job 2: HPO ResNet-50
    resnet_best_params = File('best_resnet_hpo_params.txt')
    hpo_train_resnet_file = File('hpo_train_resnet.py')
    job_hpo_train_resnet = Job(hpo_train_resnet)\
                            .add_inputs(hpo_train_resnet_file, *resized_images)\
                            .add_args(hpo_train_resnet_file, '--trials', RESNET_NUM_TRIALS)\
    ...
    train_resnet_file = File("train_resnet.py")
    job_train_resnet = Job(train_resnet)\
                        .add_args(train_resnet_file)\
                        .add_inputs(train_resnet_file, *resized_images, resnet_best_params)\
    ...
    resnet_inference_file = File('resnet_inference.py')
    job_resnet_inference = Job(resnet_inference)\
                            .add_args(resnet_inference_file)\
                            .add_inputs(resnet_inference_file, *resized_images, trained_resnet_model)\
    ...
    late_fusion_file = File("late_fusion.py")
    job_late_fusion = Job(late_fusion)\
                        .add_args(late_fusion_file)\
                        .add_inputs(late_fusion_file, resnet_train_output_prob, resnet_test_output_prob, bilstm_train_output_prob, bilstm_test_output_prob)\
